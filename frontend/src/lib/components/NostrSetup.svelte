<script>
  ...
  let hasStoredNsec = $state(false);

  let apiBaseUrl = $derived($baseURL);

  onMount(async () => {
    await checkStoredNsec();
  });
    ...
    try {
      ...
      if (data.success) {
        ...
        window.dispatchEvent(new CustomEvent('showToast', {
          detail: { message: 'NSEC stored successfully', type: 'success' }
        }));
      } else {
        window.dispatchEvent(new CustomEvent('showToast', {
      ...
      window.dispatchEvent(new CustomEvent('showToast', {
        detail: { message: 'Failed to store NSEC', type: 'error' }
      }));
    }
  }
  ...
  async function clearNsec() {
    try {
      const response = await fetch(`${apiBaseUrl}/api/nsec`, { method: 'DELETE' });
      const data = await response.json();
      if (data.success) {
        hasStoredNsec = false;
        window.dispatchEvent(new CustomEvent('showToast', {
          detail: { message: 'NSEC cleared successfully', type: 'success' }
        }));
      } else {
        window.dispatchEvent(new CustomEvent('showToast', {
          detail: { message: data.message, type: 'error' }
      ...
      window.dispatchEvent(new CustomEvent('showToast', {
        detail: { message: 'Failed to clear NSEC', type: 'error' }
      }));
    }
  }
</script>
    ...
    <div class="p-4">
      ...
      {#if hasStoredNsec}
        <div class="space-y-4">
          ...
          <Button color="red" onclick={clearNsec}>Clear NSEC</Button>
        </div>
      {:else}
