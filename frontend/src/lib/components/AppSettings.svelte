  ...
  async function saveSettings() {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Error saving settings:', await response.json());
...
<div class="max-w-4xl mx-auto p-6 space-y-6">
  <div class="text-center">
    ...
    <p class="text-gray-600 dark:text-gray-400">Configure your HAMSTR client settings</p>
  </div>
      ...
      <div class="space-y-4">
        <div class="space-y-2">
          ...
          <div class="flex items-center space-x-4">
            ...
            <Select class="w-24" bind:value={settings.RADIO_CLIENT_CALLSIGN[1]}>
        ...
        <div class="space-y-2">
          ...
          <div class="flex items-center space-x-4">
            ...
            />
            <span class="text-2xl font-bold">-</span>
            <Select class="w-24" bind:value={settings.RADIO_HAMSTR_SERVER[1]}>
              {#each Array(16).fill().map((_, i) => i) as ssid}
                <option value={ssid}>{ssid}</option>
              {/each}
            </Select>
